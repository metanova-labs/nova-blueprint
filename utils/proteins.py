...
def get_challenge_params_from_blockhash(block_hash: str, num_antitargets: int, include_reaction: bool = False) -> dict:
    """
    Use block_hash as a seed to pick 1 target and 'num_antitargets' antitargets
    from the 'Metanova/Proteins' dataset. Optionally also pick allowed reaction.
    Returns {'target': '...', 'antitargets': [...], 'allowed_reaction': '...'}.
    """
    ...
    if num_antitargets < 0:
        raise ValueError("num_antitargets must be non-negative.")
    ...
    unique_indices = rng.sample(range(dataset_size), k=num_antitargets + 1)

    # First index is the target; the rest are antitargets
    target_idx = unique_indices[0]
    antitarget_indices = unique_indices[1:]

    # Convert indices to protein codes
    target = dataset[target_idx]["Entry"]
