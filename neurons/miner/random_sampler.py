...
from nova_miner.utils import get_smiles, find_chemically_identical
...
def generate_valid_random_molecules_batch(rxn_id: int, n_samples: int, db_path: str, subnet_config: dict, 
    ...
    if not reaction_info:
        log.error(f"Could not get reaction info for rxn_id {rxn_id}")
    ...
    if not molecules_A or not molecules_B or (is_three_component and not molecules_C):
        log.error(f"No molecules found for roles A={roleA}, B={roleB}, C={roleC}")
        return {"molecules": [None] * n_samples}
    ...
    while len(valid_molecules) < n_samples:
        ...
        #log.debug(f"Iteration {iteration}: Generating {batch_size_actual} molecules, need {needed} more")
        ...
        #log.debug(f"Batch validation: {len(batch_valid_molecules)}/{len(batch_molecules)} molecules passed")
...
def generate_molecules_from_pools(rxn_id: int, n: int, molecules_A: List[Tuple], molecules_B: List[Tuple], 
    ...
    for i in range(n):
        try:
            ...
            log.error(f"Error generating molecule {i+1}/{n}: {e}")
            mol_ids.append(None)
...
def run_sampler(n_samples: int = 1000, 
    ...
    if not reactions:
        log.error("No reactions found in the database, check db path and integrity.")
        return
    ...
    if allowed_rxn_id is not None:
        if allowed_rxn_id not in rxn_ids:
            log.error(f"Allowed reaction {allowed_rxn_id} not found in the database.")
            return {"molecules": []}

        log.info(f"Generating {n_samples} random molecules for reaction {allowed_rxn_id}")
        ...
        log.info(f"Generating {n_samples} random molecules across all {len(rxn_ids)} reactions")
        molecules = []
