...
def _build_competition_payload(config, epoch_number: int, target_proteins: list[str], antitarget_proteins: list[str]) -> dict:
    return {
        "epoch": epoch_number,
...
def submit_epoch_results(
    ...
    try:
        competition_payload = _build_competition_payload(config, epoch_number, target_proteins, antitarget_proteins)
